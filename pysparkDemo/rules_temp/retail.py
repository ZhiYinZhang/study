...
def get_city_info():
    ...
    try:
        ...
        area_code=get_area(spark,path)

        city_info=co_cust.join(area_code,["com_id","sale_center_id"])\
...
city_info.foreachPartition(lambda x: write_hbase1(x,["city","county","abcode"],hbase))
    ...
    for key in days.keys():
        ...
        for i in range(len(yieldly_types)):
            ...
            try:
                ...
                print(f"error top5  yieldly_type:{yieldly_type} {key}:{day}")
                tb.print_exc()
...
def get_cust_index():
    ...
    coordinate = get_coordinate(spark,path)
...
def get_consume_level():
    ...
    try:
        ...
        consume_level_df = city_rent_avg.join(city_food_avg, ["city", "cust_id"]) \
            .join(city_hotel_avg, ["city", "cust_id"]) \

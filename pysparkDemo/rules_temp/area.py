...
co_co_01 = spark.sql("select  cast(qty_sum as float),cast(amt_sum as float),born_date,sale_center_id from DB2_DB2INST1_CO_CO_01 where (unique_kind = 90 and pmt_status=0) or (unique_kind=10 and pmt_status=1)") \
...
try:
    ...
    del (ring_order_sum)
    del (ring_amount_total)
    del (ring_avg_price)
    del (ring_order_count)
    del (order_sum_ring_ratio)
    del (amount_ring_ratio)
    del (avg_price_ring_ratio)
    del (order_count_ring_ratio)
except Exception as e:
    tb.print_exc()
...
#-------------co_co_01  去年  同比情况
last_year = spark.sql("select  cast(qty_sum as float),cast(amt_sum as float),sale_center_id,born_date from DB2_DB2INST1_CO_CO_01 where (unique_kind = 90 and pmt_status=0) or (unique_kind=10 and pmt_status=1)") \
    .withColumn("born_date", f.to_date("born_date", "yyyyMMdd")) \
...
try:
    del(last_year)
    del(last_month_qty_sum)
    del(last_year_qty_sum)
    del(last_month_amt_sum)
    del(last_year_amt_sum)
    del(last_month_avg_price)
    del(last_year_avg_price)
    del(last_month_order_count)
    del(last_year_order_count)
    del(co_co_01)
except Exception as e:
    tb.print_exc()
...
# -------------------获取co_co_line
co_co_line = spark.sql("select item_id,cast(qty_ord as float),cast(price as float),sale_center_id,born_date from DB2_DB2INST1_CO_CO_LINE") \
    .withColumn("born_date", f.to_date("born_date", "yyyyMMdd")) \
    .withColumn("today", f.current_date()) \
    .withColumn("week_diff", week_diff("today","born_date"))\
    .withColumn("month_diff",month_diff_udf(f.year(col("born_date")),f.month(col("born_date")),f.year(col("today")),f.month(col("today"))))\
...
line_plm = co_co_line.join(plm_item, "item_id").coalesce(10)
...
# 城市 city   县或区  county
try:
    area_code.groupBy("city","sale_center_id").agg(f.collect_list("county").alias("county"))\
...
for city in cities.keys():
    ...
    try:
        ...
        gdp_sum.withColumn("GDP", f.round(col("sum(gdp)") / max_gdp*5, 4)) \
            .foreachPartition(lambda x: write_hbase1(x, ["GDP"],hbase))
    except Exception as e:
        print(e.args)

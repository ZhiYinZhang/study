...
def one_card_multi_cust():
    try:
        ...
        try:
            ...
            co_co_line =get_co_co_line(spark).select("cust_id","co_num","born_date","today") \
...
def get_last_order_except():
    try:
        ...
        # -----零售店单次订货条均价异常
        try:
            avg_price = qty_amt_sum.withColumn("avg_price", divider_udf(col("amt_sum"), col("qty_sum"))) \
                .select("com_id", "cust_seg", "cust_id", "avg_price")
    ...
    try:
        ...
        try:
            ...
            out_prov_ratio=out_prov_num.join(item_num,"cust_id")\
                     .withColumn("out_prov_ratio",divider_udf(col("out_prov_num"),col("item_num")))\
                     .join(co_cust,"cust_id") \
...
def get_last_item_except():
    try:
        ...
        try:
            ...
            # 每个零售户省外烟占比
            out_prov_ratio = out_prov_num.join(item_num, "cust_id") \
                .withColumn("last_out_prov_ratio", divider_udf(col("out_prov_num"), col("item_num"))) \
                .join(co_cust, "cust_id") \
        ...
        try:
            ...
            # 每个零售户高价烟数量/总量
            high_price_ratio = item_num.join(high_price_num, "cust_id") \
                .withColumn("last_high_price_ratio", divider_udf(col("high_price_num"), col("item_num"))) \
                .join(co_cust, "cust_id") \
...
3.计算每个零售户周边订购量前三的烟one_km_rank3
   3.1 获取每个零售户经纬度cust_lng_lat， 列:cust_id0 longitude latitude
   3.2 计算每个零售户周边(1km)的经纬度范围 cust_lng_lat0,  列：cust_id1 lng_l lng_r lat_d lat_u
   3.3 计算每个零售户周边包括哪些其他零售户，cust_lng_lat和cust_lng_lat0 join在一起,条件 lng_l<longitude<lng_r and lat_d<latitude<lat_u
   列: cust_id0  cust_id1   ，零售户cust_id1周边包含cust_id0这些零售户,再和cust_item_sum表 join获取cust_id0这些零售户的每类烟的订购量
...
def get_one_km_item_except():
    try:
        ...
        """
        #3.3&3.4 每个零售户一公里内所定烟的前三
        one_km_rank3 = cust_lng_lat.join(cust_lng_lat0, (col("longitude") >= col("lng_l")) & (col("longitude") <= col("lng_r")) & (col("latitude") >= col("lat_d")) & (col("latitude") <= col("lat_u"))) \
            .join(cust_item_sum, col("cust_id0") == col("cust_id")) \
...
def get_avg_cons_level():
    try:
        ...
        avg_consume_level=avg_price.join(consume_level_df,"cust_id")\
                 .withColumn("retail_price_cons",divider_udf(col("avg_price"),col("consume_level")))
...
def get_high_cons_level():
    try:
        ...
        high_consume_level=high_price_ratio.join(consume_level_df,"cust_id")\
                        .withColumn("retail_high_cons",divider_udf(col("high_price_ratio"),col("consume_level")))

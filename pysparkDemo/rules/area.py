...
def get_order_stats_info():
    ...
    try:
        colNames = ["qty_sum", "amt_sum", "sale_center_id", "week_diff", "month_diff"]
        ...
        cols0 = {
            "sum": ["sum_last_week", "sum_last_two_week", "sum_last_four_week", "sum_last_month", "sum_this_month"],
...
def get_ratio():
    try:
        # -------------------获取co_co_line
        colNames = ["item_id", "qty_ord", "price", "sale_center_id", "week_diff", "month_diff"]
        ...
        days_total = {"1_week": total_week_1, "1_month": total_month_1, "3_month": total_month_3,
...
def get_ppl_areaCpt_index():
    ...
    try:
        # 区域零售户数目
        cust_num = spark.sql(
            "select * from DB2_DB2INST1_CO_CUST where dt=(select max(dt) from DB2_DB2INST1_CO_CUST)").select(
            "sale_center_id") \
...
def get_area_info():
    try:
        area_code = get_area(spark).where(col("city").rlike("邵阳市|岳阳市")) \
            .select("county", "sale_center_id")

        cols = ["county", "gdp", "gdp_add", "consumption", "farmer", "urban", "primary_industry",
                "primary_industry_add",
        ...
        result = get_city_info(spark).where(col("mtime") == f"{dt.now().year-1}-12-01") \
            ...
            .join(area_code, "county") \
            .groupBy("sale_center_id") \
            .agg(f.sum("gdp").alias("gdp"), f.sum("gdp").alias("gdp"), f.sum("consumption").alias("consumption"),
                 f.sum("farmer").alias("farmer"),
                 f.sum("urban").alias("urban"),
                 f.sum("primary_industry").alias("primary_industry"),
                 f.sum("primary_industry_add").alias("primary_industry_add"),
                 f.sum("second_industry").alias("second_industry"),
                 f.sum("second_industry_add").alias("second_industry_add"),
                 f.sum("tertiary_industry").alias("tertiary_industry"),
                 f.sum("tertiary_industry_add").alias("tertiary_industry_add")
                 )

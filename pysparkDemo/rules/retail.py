...
# --------------------------------------------------------retail（零售户信息）基本信息---------------------------------------------
def get_cust_info():
    co_cust_cols = ["cust_id", "cust_name", "cust_seg", "status", "pay_type", "license_code", "manager",
                    "identity_card_id",
                    "order_tel", "inv_type", "order_way", "periods", "busi_addr", "work_port", "base_type",
                    "sale_scope",
                    "scope", "com_chara", "is_tor_tax", "is_sale_large", "is_rail_cust", "area_type", "is_sefl_cust",
                    "is_func_cust",
                    "dt"]
    # 需要修改的列名   与hbase的列名对应
    ...
    try:
        ...
        cust_info = crm_cust.join(co_cust, "cust_id")
...
def get_30_grade_change():
    ...
    try:
        ...
        colName = "grade_abs"
        cust_seg.groupBy("cust_id") \
            .agg(grade_diff_udf(f.max("change_frm"), f.min("change_frm"), f.max("change_to"), f.min("change_to")).alias(
            colName)) \
            .foreachPartition(lambda x: write_hbase1(x, [colName], hbase))
        ...
        for y in range(len(types)):
            try:
                ...
                count_df = coordinate.where(col("types").rlike(regex)) \
                    .join(cust_lng_lat0,
                          (col("lng") >= col("lng_l")) & (col("lng") <= col("lng_r")) & (col("lat") >= col("lat_d")) & (
                                      col("lat") <= col("lat_u"))) \

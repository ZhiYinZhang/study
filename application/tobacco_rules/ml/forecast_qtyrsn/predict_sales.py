...
from ml.forecast_qtyrsn.utils import *
from ml.forecast_qtyrsn.train_model import *
import datetime
import gc
import pandas as pd
...
def predict(df,n):
    ...
    ts_num_feat_cols = [x for x in pad_df.columns if x not in ['item_id','born_date_week','time']+not_ts_feat]
    y_df = create_y(train_pad_df,not_ts_feat)
    train_pad_df = create_rolling_feat(train_pad_df, ts_num_feat_cols, win=5, n=n)
    train_df = y_df.merge(train_pad_df[[x for x in train_pad_df.columns if x not in not_ts_feat]], on=['item_id','born_date_week'], how='left')
    ...
    feat_cols = [x for x in test_df.columns if x not in ['item_id','born_date_week']]
    ...
    test_df['val_pred'] = np.exp(val_model.predict(test_df[val_feats], num_iteration=val_model.best_iteration_))-1
    
    # tranform results
    sub_df = test_df[test_df.predict_date>max_date][['item_id','predict_date','will_purchase','val_pred']]
    sub_df['prediction'] = (sub_df['will_purchase']*sub_df['val_pred']).astype('int')
    sub_df['item_id'] = sub_df['item_id'].map(inv_item_mapping)
    sub_df = pd.pivot_table(sub_df, columns='predict_date', values='prediction', index='item_id')
...
if __name__ == '__main__':
    ...
    pd_df = pd.read_parquet(data_path)
    # 需要co_co_line的
    # cust_id           object
    # co_num            object
    # line_num          float64
    ...
    n = 1
    sub_df = predict(pd_df,n)
    print(sub_df)
    df=spark.createDataFrame(sub_df)
    df.show()
    # sub_df.to_csv("e://test/"+'zhuzhou_qtyrsn_'+datetime.datetime.strftime(datetime.datetime.today(), format='%Y%m%d')+'.csv',index=False,sep=',')

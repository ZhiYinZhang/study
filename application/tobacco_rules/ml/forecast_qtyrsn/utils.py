...
def create_basic_feat(df):
    ...
    train_df = df.groupby(['item_id','born_date_week'],as_index=False).agg({
    ...
    cols = ['item_id','born_date_week']+[x+'_'+y for x,y in list(product(['qty_need','qty_ord','qty_rsn','need_ord_diff','rsn_ord_diff'],
                                 ['sum','max','min','median','mean','std','skew','kurt']))]
    train_df.columns = cols
    
    feat_df = df.groupby(['item_id','born_date_week'],as_index=False).agg({
    'is_purchase':['sum','mean'],
    'ord_less_rsn':['sum','mean'],
    'ord_less_need':['sum','mean'],
    })
    feat_df.columns = ['item_id','born_date_week']+[x+'_'+y for x,y in list(product(['is_purchase','ord_less_rsn','ord_less_need'],
                                     ['sum','mean']))]
    train_df = train_df.merge(feat_df, on=['item_id','born_date_week'], how='left')
    
    # 当周有记录、有下单、没下单、退货的零售户数量
    pur_cust = df.groupby(['item_id','born_date_week','cust_id'],as_index=False).agg({'qty_ord':['sum']})
    pur_cust.columns = ['item_id','born_date_week','cust_id'] + [x+'_'+y for x,y in list(product(['qty_ord'],['sum']))]
    pur_cust['cust_purchase'] = pur_cust['qty_ord_sum']>0
    pur_cust['cust_return'] = pur_cust['qty_ord_sum']<0
    pur_cust_count = pur_cust.groupby(['item_id','born_date_week'],as_index=False).agg({'cust_purchase':['sum','count'],'cust_return':['sum']})
    pur_cust_count.columns = ['item_id','born_date_week']+[x+'_'+y for x,y in list(product(['cust_purchase'],['sum','count']))]+['cust_return_sum']
    ...
    # 拼接主dataframe
    train_df = train_df.merge(pur_cust_count, on=['item_id','born_date_week'], how='left')
    
    
    # 零售户平均购买间隔与当周距离上次记录间隔的差
    interval_df = df[['item_id','cust_id','born_date_week','qty_rsn','qty_ord']].sort_values(['item_id','cust_id','born_date_week'])
    interval_df = interval_df.dropna()
    
    # 零售户平均购买间隔
    pur_int_df = interval_df[interval_df['qty_ord']>0]
    pur_int_df['last_pur'] = pur_int_df.groupby(['item_id','cust_id'],as_index=False)['born_date_week'].shift(1)
    ...
    pur_int_df = pur_int_df.groupby(['item_id','cust_id'],as_index=False).agg({'pur_interval':'mean'})
    interval_df = interval_df.merge(pur_int_df, on=['item_id','cust_id'], how='left')
    
    # 零售户当周距离上次记录间隔
    interval_df['last_record'] = interval_df.groupby(['item_id','cust_id'],as_index=False)['born_date_week'].shift(1)
    ...
    # 聚合为供需特征
    interval_df = interval_df=interval_df.groupby(['item_id','born_date_week'],as_index=False).agg({
    ...
    cols = ['item_id','born_date_week']+[x+'_'+y for x,y in list(product(['pur_interval','record_interval','diff_pur_interval_record_interval'],
                                     ['sum','max','min','median','mean','std','skew','kurt']))]
    interval_df.columns = cols
    
    # 拼接主dataframe
    train_df = train_df.merge(interval_df, on=['item_id','born_date_week'], how='left')
...
def pad_ts(df):
    ...
    all_df=pd.DataFrame(list(map(list,itertools.product(item,time))), columns=['item_id','born_date_week'])
    pad_df = all_df.merge(df, on=['item_id','born_date_week'],how='left')
    pad_df = pad_df.sort_values(['item_id','born_date_week'])
    return pad_df

def create_holiday_feat(holiday_df, df, tm_col='born_date_week'):
    # 节假日
    pad_train_df = df.copy()
    pad_train_df = pd.merge_asof(pad_train_df.sort_values(tm_col), holiday_df, 
...
def get_num_feat(feat,X_df,win):
    X_df[feat+'_rolling_5_mean'] = X_df.groupby('item_id',as_index=True)[feat].rolling(window=win,min_periods=1).mean().values
    X_df[feat+'_rolling_5_min'] = X_df.groupby('item_id',as_index=True)[feat].rolling(window=win,min_periods=1).min().values
    X_df[feat+'_rolling_5_max'] = X_df.groupby('item_id',as_index=True)[feat].rolling(window=win,min_periods=1).max().values
    X_df[feat+'_rolling_5_max_minus_min'] = X_df.groupby('item_id',as_index=True)[feat].rolling(window=win,min_periods=1).\
                                            apply(max_minus_min).values
    X_df[feat+'_rolling_5_mean_minus_min'] = X_df.groupby('item_id',as_index=True)[feat].rolling(window=win,min_periods=1).\
                                            apply(mean_minus_min).values
    X_df[feat+'_rolling_5_mean_minus_max'] = X_df.groupby('item_id',as_index=True)[feat].rolling(window=win,min_periods=1).\
...
def create_expected_historical_feat(holiday_df,df,tm_col='born_date_week'):
...
def create_y(df,feat_cols):
    ...
    y_df = df[['item_id','born_date_week','qty_rsn_sum']+feat_cols].fillna(0)
    y_df.rename(columns={'qty_rsn_sum':'label'},inplace=True)
    # print('y_df columns:',y_df.columns)
    y_df['log_label'] = np.log(y_df['label']+1)

...
def reality_supply():
    ...
    columns=["item_id","item_name","city","gears_date_marker"]+cols
...
def supply_analysis_city():
    ...
    try:
        ...
        columns=["city","ciga_id","ciga_gauge","gears_data_marker",colName]
        ...
        top5 = reserve_ratio.withColumn("rank", f.row_number().over(win)) \
            .where(col("rank") <= 5) \
            .join(plm_item, "item_id") \
            .withColumn("json", json_udf(col("item_id"), col("item_name"))) \
            .groupBy("com_id") \
    ...
    try:
        ...
        #上上周
        last_two_week=get_co_co_line(spark,scope=[2,2],filter="week").select("com_id","cust_id","item_id","qty_ord")
    ...
    try:
        ...
        columns = ["city","cust_id","cust_name","longitude","latitude", "ciga_id", "ciga_gauge","gears_data_marker", colName]
...
def supply_analysis_county():
    ...
    try:
        ...
        columns = ["sale_center_id","county", "ciga_id", "ciga_gauge", "gears_data_marker",colName]

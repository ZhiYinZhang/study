...
from rules.config import block_path,block_data_table
...
hbase={"table":block_data_table,"families":["0"],"row":"block_id"}

#区域卷烟主销品规（省内、省外烟）
def get_block_item_top():

    try:
        cities={"岳阳市":"011114306","株洲市":"011114302"}
        #城市分块数据
        block_data=spark.read.csv(block_path,header=True) \
            ...
            .select("city","block_id", "lng1", "lat1", "lng0", "lat0")
        ...
        #近30天订单行表数据
        co_co_line = get_co_co_line(spark, [0, 30]).select("cust_id", "item_id", "qty_ord", "qty_rsn")
        ...
        for city in cities.keys():
            com_id=cities[city]
            print(f"{str(dt.now())} {city} 区域卷烟主销品规")
            #城市分块数据
            city_block_data =block_data.where(col("city")==city)
            ...
            for i in range(len(provs)):
                ...
                co_co_line0=co_co_line.where(col("com_id") == com_id) \
                                    .join(plm_item.where(col("yieldly_type") == prov), "item_id") \
                                    .select("cust_id", "item_id", "qty_ord", "qty_rsn")


                # 每个零售户各类烟的订单量  qty_rsn>0
                cust_item_qty = co_co_line0.groupBy("cust_id", "item_id") \
                ...
                #每个区块的零售户 烟
                block_item = city_block_data.join(cust_lng_lat0, (col("lng") > col("lng1")) & (col("lng") < col("lng0")) & (
...
def get_block_item_top1():
    try:
        ...
        cust_block_data = block_data.join(cust_lng_lat,
                                          (col("city") == col("city0")) &
                                          (col("lng") > col("lng1")) & (col("lng") < col("lng0")) &
                                          (col("lat") > col("lat1")) & (col("lat") < col("lat0"))
                                          ).select("city", "block_id", "cust_id")
        cust_block_data.cache()
        ...
        for city in cities.keys():
            ...
            print(f"{str(dt.now())} {city} 区域卷烟主销品规")
            # 城市分块数据
            city_block_data = cust_block_data.where(col("city") == city)
            ...
            for i in range(len(provs)):
                ...
                item_qty_std = cust_item_qty.join(max_min, "cust_id") \
                    .withColumn("item_qty_ord_std",
                                (col("qty_ord") - col("ord_min")) / (col("ord_max") - col("ord_min"))) \
                    .withColumn("item_qty_rsn_std",
                                (col("qty_rsn") - col("rsn_min")) / (col("rsn_max") - col("rsn_min")))

                # 每个区块的零售户 烟
                block_item = city_block_data \
                    .join(item_qty_std, "cust_id") \
                    .select("block_id", "cust_id", "item_id", "item_qty_ord_std", "item_qty_rsn_std")
                ...
                # 4.每个区域 卷烟订足率   基于 1   在规定的区域内，各卷烟实际订购量/该卷烟合理订购量。
                enough_rate = block_item_num.withColumn("enough_rate",
                                                        col("block_item_ord_num") / col("block_item_rsn_num")) \
                    .select("block_id", "item_id", "enough_rate")
                ...
                score = coverage_rate.join(item_ratio, ["block_id", "item_id"]) \
                    .join(enough_rate, ["block_id", "item_id"]) \
                    .withColumn("score",
                                col("item_ratio") * 0.4 + col("enough_rate") * 0.4 + col("coverage_rate") * 0.2) \
                    .select("block_id", "item_id", "score")
